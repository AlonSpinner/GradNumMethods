function [xStruct,optimValues,changed] = outputFcn_global_sa(optimValues,xStruct,state)
% outputFcn_global()
%
...
switch state
  case 'init'
    ...
    outputFcn_global_data.Score = xStruct.Score;
    outputFcn_global_data.Population = xStruct.Population;
    outputFcn_global_data.optimValues = optimValues;
    outputFcn_global_data.timerVal = tic;
  case 'iter'
    ind = length(outputFcn_global_data)+1;
    outputFcn_global_data(ind).Score = xStruct.Score;
    outputFcn_global_data(ind).Population = xStruct.Population;
